...
# a simple way to have multipl single-head attention in parallel is to stack multiple CausalAttention modules in a wrapper module
class MultiHeadAttentionWrapper(nn.Module):
    ...
    def forward(self, x):
        # this processes the heads sequentially
...
# this (re)implementation of multihead attention class takes advantage of
# transpose and reshape of tensors to allow matrix multiplication.
# the calculations are not sequential anymore, and we only need only one matrix multiplication, which is more efficient! 
class MultiHeadAttention(nn.Module):
    def __init__(self, d_in, d_out,
        ...
        self.head_dim = d_out // num_heads # mattch the desired output dimension
        self.W_query = nn.Linear(d_in, d_out, bias=qkv_bias)
        self.W_key = nn.Linear(d_in, d_out, bias=qkv_bias)
        self.W_value = nn.Linear(d_in, d_out, bias=qkv_bias)
        self.out_proj = nn.Linear(d_out, d_out) # Linear layer combining the head outputs
    ...
    def forward(self, x):
        ...
        queries = self.W_query(x)
        values = self.W_value(x)
        # split the matrix by adding num_heads dimension and unroll last dimension
        ...
        # context vectors from all heads are transposed back to the original (flattened) shape
        context_vec = context_vec.contiguous().view(
            b, num_tokens, self.d_out
        )
        # add linear projection (optional, commonly used in LLM architectures)
